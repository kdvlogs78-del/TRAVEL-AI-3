/* ============================================================
   PLAN YOUR TRIP INDIA — Desktop JavaScript
   AI Chatbot · Clickable India Map · Auth System
   ============================================================ */
...
function toggleTheme() {
  ...
  localStorage.setItem('rasa-theme', newTheme);
}

// Init theme from storage
(function initTheme() {
  const saved = localStorage.getItem('rasa-theme') || 'dark';
...
const CITY_DATA = {
  ...
  ladakh: {
    ...
    hotels: ['Grand Dragon Ladakh (Luxury)', 'Zostel Leh (Budget)', 'Stok Palace Heritage', 'The Indus Valley'],
...
function changeTravelers(delta) {
  ...
  document.getElementById('travCount').textContent = travelerCount;
}
...
function initIndiaMap() {
  ...
  fetch('https://raw.githubusercontent.com/geohacker/india/master/state/india_state.geojson')
    ...
    .then(data => {
      stateLayer = L.geoJSON(data, {
        ...
        onEachFeature: (feature, layer) => {
          ...
          layer.on({
            mouseover: e => {
              e.target.setStyle({ fillColor: '#FEFACD', fillOpacity: 0.38, color: '#FEFACD', weight: 2 });
    ...
    .catch(() => console.log('India GeoJSON failed to load'));
}

function getStateStyle() {
  return {
    fillColor: 'rgba(123,98,184,0.28)',
    fillOpacity: 1,
...
function triggerStateAI(stateName) { onStateClick(stateName); }

function chatAboutState() {
  openChat();
  setTimeout(() => {
    const msg = `Tell me more about travelling in ${currentStateClicked}, India — best hidden gems, must-try local experiences and a suggested 5-day itinerary.`;
...
function planThisState() {
  document.getElementById('destination').value = currentStateClicked;
...
async function generatePlan() {
  ...
  btn.querySelector('.gen-btn-text').style.display = 'none';
  btn.querySelector('.gen-btn-loader').style.display = 'flex';
  btn.disabled = true;

  const cityKey = Object.keys(CITY_DATA).find(k => destination.toLowerCase().includes(k));
  const cityData = cityKey ? CITY_DATA[cityKey] : null;

  try {
    const prompt = `Create a ${duration}-day travel itinerary for ${destination}, India for ${travelerCount} traveler(s).
Budget level: ${budgetRange}. Travel style: ${style}.

Respond ONLY with valid JSON in this exact structure:
{
  "city": "${destination}",
  "tagline": "one evocative line about this destination",
  "famous": ["place1","place2","place3","place4","place5"],
  "hidden": ["gem1","gem2","gem3","gem4"],
  "food": [{"name":"dish1","price":"₹XX"},{"name":"dish2","price":"₹XX"},{"name":"dish3","price":"₹XX"}],
  "day_plan": [
    {"day":1,"title":"Day title","morning":"morning activity","afternoon":"afternoon activity","evening":"evening activity","food":"food recommendation"}
  ],
  "budget": {"accommodation":XXXX,"food":XXXX,"transport":XXXX,"activities":XXXX},
  "tips": "one practical tip"
}
Generate for all ${duration} days. Budget numbers are per person total for trip in INR.`;
    ...
    try {
      ...
      planData = buildFallbackPlan(destination, duration, budgetRange, cityData);
    }
    renderPlanResults(planData, destination, duration, budgetRange);
  } catch (err) {
    const fallback = buildFallbackPlan(destination, duration, budgetRange, cityData);
    renderPlanResults(fallback, destination, duration, budgetRange);
  } finally {
    btn.querySelector('.gen-btn-text').style.display = 'flex';
    btn.querySelector('.gen-btn-loader').style.display = 'none';
    btn.disabled = false;
  }
}

function buildFallbackPlan(destination, days, budgetRange, cityData) {
  const d = cityData || {
  ...
  const perDay = d.per_day ? d.per_day[budgetRange] || 3000 : 3000;
  ...
  return {
    ...
    famous: d.famous, hidden: d.hidden, food: d.food, day_plan: plan,
    budget: {
...
function renderPlanResults(plan, destination, duration, budgetRange) {
  document.getElementById('plannerFormArea').style.display = 'none';
  document.getElementById('plannerResults').style.display = 'block';
  document.getElementById('resultTitle').textContent = `Your ${plan.city || destination} Plan`;
  document.getElementById('resultSubtitle').textContent = `${duration} days · ${budgetRange} · ${travelerCount} traveler(s)`;
  ...
  grid.innerHTML = `
    <div class="result-card">
      ...
      <ul>${(plan.famous || []).map(p => `<li>${p}</li>`).join('')}</ul>
    </div>
    <div class="result-card">
      <h4>💎 Hidden Gems</h4>
      <ul>${(plan.hidden || []).map(p => `<li>${p}</li>`).join('')}</ul>
    </div>
    <div class="result-card">
      <h4>🍜 Must-Try Food</h4>
      <ul>${(plan.food || []).map(f => `<li>${f.name} <span style="color:var(--gold);margin-left:auto">${f.price || ''}</span></li>`).join('')}</ul>
    </div>
    <div class="result-card">
    ...
    ${plan.tips ? `<div class="result-card result-card-full">
      <h4>💡 Local Tips</h4>
      <p style="color:var(--text-muted);font-size:.9rem;line-height:1.7">${plan.tips}</p>
    </div>` : ''}
  `;

  document.getElementById('plannerResults').scrollIntoView({ behavior: 'smooth', block: 'start' });
}

function resetPlanner() {
  document.getElementById('plannerFormArea').style.display = 'block';
...
function openCityPage(key) {
  ...
  document.querySelectorAll('.city-tab-content').forEach((c, i) => c.classList.toggle('active', i === 0));
}
...
function showTypingIndicator() {
  ...
  div.innerHTML = `
    ...
    <div class="msg-content"><div class="ai-typing-dots"><span></span><span></span><span></span></div></div>
  `;
  msgs.appendChild(div);
  msgs.scrollTop = msgs.scrollHeight;
  return id;
...
try { currentUser = JSON.parse(localStorage.getItem('pyti_user') || 'null'); } catch (e) { }
...
function clearAuthErrors(modalId) {
  ...
  if (!modal) return;
  modal.querySelectorAll('.auth-error,.auth-success').forEach(el => { el.style.display = 'none'; el.textContent = ''; });
...
function handleLogin() {
  ...
  if (!email || !password) { showAuthError('loginModal', 'loginError', '⚠ Please fill in all fields.'); return; }
  if (!/\S+@\S+\.\S+/.test(email)) { showAuthError('loginModal', 'loginError', '⚠ Please enter a valid email address.'); return; }
  ...
  setTimeout(() => {
    ...
    try {
      ...
      showAuthError('loginModal', 'loginError', '✕ Login failed. Please try again.');
    }
  }, 1200);
}
...
function handleSignup() {
  ...
  if (!first || !last || !email || !password) { showAuthError('signupModal', 'signupError', '⚠ Please fill in all fields.'); return; }
  if (!/\S+@\S+\.\S+/.test(email)) { showAuthError('signupModal', 'signupError', '⚠ Please enter a valid email address.'); return; }
  if (password.length < 8) { showAuthError('signupModal', 'signupError', '⚠ Password must be at least 8 characters.'); return; }
  ...
  setTimeout(() => {
    ...
    try {
      const users = JSON.parse(localStorage.getItem('pyti_users') || '[]');
      if (users.find(u => u.email === email)) { showAuthError('signupModal', 'signupError', '✕ An account with this email already exists.'); return; }
      ...
      showAuthError('signupModal', 'signupError', '✕ Sign up failed. Please try again.');
    }
  }, 1400);
}
...
function logoutUser() {
  currentUser = null;
  try { localStorage.removeItem('pyti_user'); } catch (e) { }
  refreshNavAuth();
